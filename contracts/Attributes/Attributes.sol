...
import {Stats, StatsEnum} from "../Interfaces/GameObjects/IGameObjects.sol";
...
contract Attributes is Initializable, InitNavigator {
    ...
    mapping(uint => Stats) public SummonerStats;
    ...
    function allocate(uint summoner, Stats memory _stats) external ensureNotPaused senderIsSummonerOwner(summoner) {
        ...
        SummonerStats[summoner] = Stats({STR : _stats.STR * 10, AGI : _stats.AGI * 10, DEX : _stats.DEX * 10, INT : _stats.INT * 10, VIT : _stats.VIT * 10, LUCK : _stats.LUCK * 10});
    ...
    function reset(uint summoner) external ensureNotPaused senderIsSummonerOwner(summoner) {
        ...
        SummonerStats[summoner] = Stats(0, 0, 0, 0, 0, 0);
        UsedPoints[summoner] = 0;
        UsedLevelPoints[summoner] = 0;
    }

    function increaseStat(uint summoner, StatsEnum stat) external ensureNotPaused senderIsSummonerOwner(summoner) {
    ...
    function statsOfSummoners(uint[] memory ids) external view returns (Stats memory _stats) {
        //        if (UsedPoints[summoner] < 1) revert AlreadyAllocated(summoner, "NOT ALLOCATED");
