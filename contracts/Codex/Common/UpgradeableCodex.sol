import {ICodexPrefixAndSuffix} from "../../Interfaces/Codex/ICodexPrefixAndSuffix.sol";
import {InitNavigator, INavigator} from "../../Core/Navigator/InitNavigator.sol";
import {Prefix, Suffix} from "../../Interfaces/GameObjects/IGameObjects.sol";

pragma solidity ^0.8.0;

contract UpgradeableCodex is InitNavigator {
    ICodexPrefixAndSuffix PrefixContract;
    ICodexPrefixAndSuffix SuffixContract;


    function initialize(address _navigator) external initializer {
        initializeNavigator(_navigator);
    }

    function initializePSContracts() external {
        PrefixContract = ICodexPrefixAndSuffix(contractAddress(INavigator.CONTRACT.PREFIX_CODEX));
        SuffixContract = ICodexPrefixAndSuffix(contractAddress(INavigator.CONTRACT.SUFFIX_CODEX));
    }

    function prefixAndSuffix(uint _prefix, uint _prefixTier, uint _suffix, uint _suffixTier) internal view returns (Prefix memory, Suffix memory) {
        Prefix memory prefix = PrefixContract.prefix(_prefix, _prefixTier);
        Suffix memory suffix = SuffixContract.suffix(_suffix, _suffixTier);

        return (prefix, suffix);
    }
}
