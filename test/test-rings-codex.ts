...
describe("Weapon Codexs", () => {
    it('Should deploy navigator.', async () => {
        ...
        // rings
        CodexRings = await ethers.getContractFactory("CodexRings")
        codexRings = await upgrades.deployProxy(CodexRings);
        ...
        tx = await navigator.setGameContractsById(CONTRACT.RINGS_CODEX, codexRings.address, true)
        await tx.wait(1)
        console.log("and set in navigator.")


        fs.writeFileSync('./test/rings-codex.json', JSON.stringify({
