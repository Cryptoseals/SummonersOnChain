...
describe("Weapon Codexs", () => {
    it('Should deploy navigator.', async () => {
        ...
        // belts
        CodexBelts = await ethers.getContractFactory("CodexBelts")
        codexBelts = await upgrades.deployProxy(CodexBelts, [navigator.address], {
            initializer: "initialize"
        });
        await codexBelts.deployed()
        console.log("CodexBelts deployed to:", codexBelts.address,)
        tx = await navigator.setGameContractsById(CONTRACT.BELTS_CODEX, codexBelts.address, true)
        await tx.wait(1)
        console.log("and set in navigator.")


        fs.writeFileSync('./test/belts-codex.json', JSON.stringify({
