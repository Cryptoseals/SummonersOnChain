...
describe("Weapon Codexs", () => {
    it('Should deploy navigator.', async () => {
        ...
        // amulets
        CodexAmulets = await ethers.getContractFactory("CodexAmulets")
        codexAmulets = await upgrades.deployProxy(CodexAmulets);
        ...
        tx = await navigator.setGameContractsById(CONTRACT.RINGS_CODEX, codexAmulets.address, true)
        await tx.wait(1)
        console.log("and set in navigator.")


        fs.writeFileSync('./test/amulets-codex.json', JSON.stringify({
